#ifndef QCSNN24_RR_SD_H
#define QCSNN24_RR_SD_H
...
static const int INPUT_LEN = SIGNAL_LEN + RR_DIM;               // 184
...
// CHANGED: Full argmax over all 4 classes (Stage 2 can predict Normal)
 ...
 *   - multi head (net4) gets trunk only (480)
 * =================================================================== */
template<int NUM_STEPS>
class QCSNN24_RR_SD {
public:
    QCSNN24_RR_SD()  = default;
    ~QCSNN24_RR_SD() = default;
...
#pragma HLS INTERFACE s_axilite port=return bundle=CTRL
        ...
        /* ----------------------------------------------------------
         * 0) Buffer ONE record (184 samples: 180 signal + 4 RR)
         * ---------------------------------------------------------- */
        ap_int8_c sig_buf[SIGNAL_LEN];   // 180 signal samples
#pragma HLS ARRAY_PARTITION variable=sig_buf cyclic factor=8 dim=1

        ap_int8_c rr_buf[RR_DIM];        // 4 RR features
#pragma HLS ARRAY_PARTITION variable=rr_buf complete dim=1
...
#pragma HLS STREAM variable=s_body depth=TRUNK_OUT
        ...
        // binary head streams (484 -> 2) -- CHANGED from 480
        hls::stream<ap_int8_c> s_bin_qi("s_bin_qi");
#pragma HLS STREAM variable=s_bin_qi depth=STAGE1_IN
...
#pragma HLS STREAM variable=s_bin_out depth=2
        ...
        // multi head streams (480 -> 128 -> 4) -- UNCHANGED
        hls::stream<ap_int8_c> s_m_qi1("s_m_qi1");
#pragma HLS STREAM variable=s_m_qi1 depth=TRUNK_OUT
        hls::stream<ap_int8_c> s_m_fc1("s_m_fc1");
#pragma HLS STREAM variable=s_m_fc1 depth=TRUNK_OUT
...
#pragma HLS STREAM variable=s_m_out depth=FIXED_LENGTH9
        ...
        /* ----------------------------------------------------------
         * 3) STAGE-1: trunk + binary head (with RR) + cache body_t
         * ---------------------------------------------------------- */
    STAGE1_LOOP:
...
#pragma HLS PIPELINE II=1
            ...
            // (b) trunk (net24) -- processes 180 signal samples
            trunk_conv1.forward(
                s_in, s0,
            ...
            // (c) cache body + feed binary head with trunk + RR (484)
            for (int i = 0; i < TRUNK_OUT; ++i) {
#pragma HLS PIPELINE II=1
                ap_int8_c v = s_body.read();
                body_cache[t][i] = v;
                s_bin_qi.write(v);       // trunk features (480)
            }

            // Append RR features (4) to binary head input
            for (int i = 0; i < RR_DIM; ++i) {
#pragma HLS PIPELINE II=1
                s_bin_qi.write(rr_buf[i]);
            }

            // binary head (net2): qi -> linear(484->2) -> lif
        ...
        /* ----------------------------------------------------------
         * 4) STAGE-2: multi head ONLY if abnormal (NO RR features)
         * ---------------------------------------------------------- */
        multi_lif1.reset();
        multi_lif2.reset();
    ...
    STAGE2_LOOP:
        for (int t = 0; t < NUM_STEPS; ++t) {
            ...
            // feed cached body_t (480 only, no RR) into multi head
            for (int i = 0; i < TRUNK_OUT; ++i) {
#pragma HLS PIPELINE II=1
        ...
        // CHANGED: Full argmax - Stage 2 can predict Normal (self-correction)
...
private:
    /* ====================== TRUNK (net24) ====================== */
    Conv1D_SD<1, 16, 3, 1, SIGNAL_LEN>                   trunk_conv1;  // 180 input
    ...
    /* ==================== BINARY HEAD (net2) =================== */
    // CHANGED: Input size 484 (480 trunk + 4 RR)
    QuantIdentityPerTensor_Int8<STAGE1_IN, 1>            bin_qi;       // 484 flattened
    Linear1D_SD<STAGE1_IN, 2>                            bin_fc;       // 484->2
    LIF1D_SD_Integer<2, 1>                               bin_lif;

    /* ===================== MULTI HEAD (net4) =================== */
    // UNCHANGED: Input size 480 (no RR)
    QuantIdentityPerTensor_Int8<TRUNK_OUT, 1>            multi_qi1;    // 480
    Linear1D_SD<TRUNK_OUT, FIXED_LENGTH8>                multi_fc1;    // 480->128
